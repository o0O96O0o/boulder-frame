...
- Plan pan offline across the full normalized shot with `lookahead-v2` (`scipy-highs-ds`), using future accepted sampled boxes as crop constraints, never interpolating or predicting athlete positions.
...
- Use six lexicographic objectives: speed excess, acceleration excess, trapezoidal duration-weighted L1 distance outside the seed deadzone, total absolute center travel, total absolute velocity change including rest, then duration-weighted exact seed deviation as the final composition tie-break. Lock every prior optimum plus tolerance `1e-8`. Immutable `pan_dead_zone_fraction=0.05` means a per-axis radius of 5% of each seed crop's width/height around its center, normalized to source dimensions for the LP; it is a soft objective, never a containment relaxation or selective window.
...
- Keep `CropPlanner` API/storage contracts unchanged. Validate the exact immutable planner map before cached replay. Deploy backend/worker `w0.2.7` together after draining old jobs; never rewrite/retry old jobs or reuse old crop paths for the new objective.
